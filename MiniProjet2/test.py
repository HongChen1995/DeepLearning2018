...
Nreps = 5
time_elapsed = FloatTensor(Nreps).zero_()
train_err = FloatTensor(Nreps).zero_()
test_err = FloatTensor(Nreps).zero_()
...
for irep in range(Nreps):
    ...
    # Train model
    train_model(model, train_input, train_target, lossfunction, optimizer, Nepochs, b_size, log_loss=True)
    ...
    print("Train error rate {}%, test error rate {}%\n".format(100*nb_train_err/Ntrain, 100*nb_test_err/Ntest))

    time_elapsed[irep] = time.time() - t_start
    train_err[irep] = 100*nb_train_err/Ntrain
    test_err[irep] = 100*nb_test_err/Ntest

print("Initializing, training, testing time:"
      " {:4.3g}+-{:4.3g} [s]".format(time_elapsed.mean(), time_elapsed.std()))
